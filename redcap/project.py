...
class Project(object):
    ...
    @classmethod
    def create(cls, url, supertoken, data, verify_ssl = True):
        """
        Parameters
        ---------
        url : str
            API URL to your REDCap server
        supertoken: str
            API supertoken (granted by admin) for creating new projects
        data: dict
            Data object containing details about the project to be created
            ...
            note1/ see REDCap API for full list of data parameters
            note2/ 'project_title' and 'purpose' are required
            
        verify_ssl : boolean, str
            Verify SLL, default True. Can pass path to CA_BUNDLE

        WARNING: 'returnFormat' other than 'csv' appears to fail due to unknown issue
        """
        ...
        # Execute request
        response = rcr.execute(**{'verify': verify_ssl})[0]
    ...
    def export_survey_link(self, record, instrument, event=None, format='json', df_kwargs=None):
        """
    ...
    def export_survey_queue_link(self, record, format='json', df_kwargs=None):
        """
    ...
    def export_survey_return_code(self, record, instrument, event=None, format='json', df_kwargs=None):
        """
